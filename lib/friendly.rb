  ...
  def self.require gem_name, gem_install_name, script_name
    begin
      ...
      if e.message =~ /#{gem_name}/
        ...
        STDERR.puts  %Q{#{bold_red}Gem "#{gem_name}" not found by file #{script_proj}/#{script_name}#{exit_color}}
        abort        %Q{#{bold_red}Please install in #{script_proj} with "gem install #{gem_install_name}"#{exit_color}}
        raise msg
      end
      raise
    end
